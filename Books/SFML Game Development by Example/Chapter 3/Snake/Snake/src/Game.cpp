...
Game::Game() : 
	...
	m_textbox.Setup(5, 14, 350, sf::Vector2f(255.0f, 0.0f));
	m_elapsed = 0.0f;

	m_textbox.Add("Seeded random number generator with: " + std::to_string(static_cast<unsigned int>(time(nullptr))));
}
...
void Game::Update() {
	...
	float timestep = 1.0f / static_cast<float>(m_snake.GetSpeed());
	if (m_elapsed >= timestep) {
		...
		if (m_snake.HasLost()) {
			m_textbox.Add("GAME OVER! Score: " + std::to_string(static_cast<long long>(m_snake.GetScore())));
			m_snake.Reset();
		}
	}
}
